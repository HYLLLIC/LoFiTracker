...
struct DialLAF : public juce::LookAndFeel_V4
{
                           ...
                           juce::Slider& slider) override
    {
        ...
        // the thumb with a flat lo-fi look: solid fill + hard dark border.
        ...
        const float thumbR  = lineW;

        const juce::Point<float> tp (
            bounds.getCentreX() + arcR * std::sin (toAngle),
            bounds.getCentreY() - arcR * std::cos (toAngle));

        // Flat solid fill — Mikado yellow, no gradient
        g.setColour (juce::Colour (0xffFDC618));
        g.fillEllipse (tp.x - thumbR, tp.y - thumbR, thumbR * 2.0f, thumbR * 2.0f);

        // Hard dark border — chunky lo-fi outline
        g.setColour (juce::Colour (0xff111110));
        g.drawEllipse (tp.x - thumbR, tp.y - thumbR, thumbR * 2.0f, thumbR * 2.0f, 1.5f);
    }
};
